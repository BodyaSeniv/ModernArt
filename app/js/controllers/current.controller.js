    ...
    function ($scope, $state, Current, Artwork) {
      Current.getAll(function(data) {
        ...
        for(var i = $scope.currents.length; i--;){
          if ($scope.currents[i].purchaseState == 0) {
            $scope.currents[i].purchaseStateText = 'Новый'
          } else if ($scope.currents[i].purchaseState == 1) {
            $scope.currents[i].purchaseStateText = 'В работе'
          }
          if($scope.currents[i].purchaseState == 2 || $scope.currents[i].purchaseState == 3) {
